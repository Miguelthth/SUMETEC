/* Actualización segura: conserva la compra en captura; una foto seleccionada aplaza el cambio. */
(function(){
  const CLAVE='sumetec_compras_actualizacion', OMITIR=/(?:token|password|pin|codigo)/i, SELECTOR='input:not([type="file"]):not([type="password"]),textarea,select,[contenteditable="true"]'; let aplicando=false;
  const capa=m=>document.getElementById('sumetecActualizacion')?.classList.toggle('activo',!!m);
  function guardarContexto(buildDestino){if([...document.querySelectorAll('input[type="file"]')].some(el=>el.files?.length))throw Error('hay un archivo que no se puede restaurar con seguridad');const campos=[...document.querySelectorAll(SELECTOR)].filter(el=>!OMITIR.test(el.id||el.name||'')).map(el=>({id:el.id||'',name:el.name||'',tag:el.tagName,type:el.type||'',valor:el.isContentEditable?el.textContent:el.value,marcado:/checkbox|radio/.test(el.type)?el.checked:undefined}));return{schema:2,buildDestino,campos,guardadoEn:Date.now()};}
  ...
  function restaurarContexto(){const raw=sessionStorage.getItem(CLAVE);if(!raw)return false;let estado;try{estado=JSON.parse(raw)}catch(_){sessionStorage.removeItem(CLAVE);return false}if(![1, 2].includes(estado.schema)||estado.buildDestino!==VERSION_CODIGO||!Array.isArray(estado.campos)){sessionStorage.removeItem(CLAVE);return false}if(typeof iniciar==='function')iniciar();const campos=[...document.querySelectorAll(SELECTOR)].filter(el=>!OMITIR.test(el.id||el.name||''));estado.campos.forEach((dato,i)=>{const el = _destinoCampo_(campos, dato, i, estado.campos.length);if(!el||el.tagName!==dato.tag||(el.type||'')!==dato.type)return;if(el.isContentEditable)el.textContent=dato.valor;else if(/checkbox|radio/.test(el.type))el.checked=!!dato.marcado;else el.value=dato.valor;el.dispatchEvent(new Event('input',{bubbles:true}));el.dispatchEvent(new Event('change',{bubbles:true}));});sessionStorage.removeItem(CLAVE);navigator.serviceWorker.controller?.postMessage({type:'CONFIRMAR_ARRANQUE'});return true;}
